...
import { ApiResponse, WeeklyInsight, DashboardStats } from '../shared/types/api';
...
router.get('/weekly', async (_req: Request, res: Response) => {
  try {
    ...
    for (const e of entries) {
      ...
      if (dayMap[day] !== undefined) dayMap[day] += e.durationSeconds;
    }
    ...
    if (prevTotal > 0 && totalSeconds > 0) {
      ...
      if (pct > 10) aiInsight = `本周追踪时间比上周增加了 ${pct}%，工作强度有所提升，请注意劳逸平衡。`;
      else if (pct < -10) aiInsight = `本周追踪时间比上周减少了 ${Math.abs(pct)}%，建议检查是否有未记录的工作时间。`;
      else aiInsight = `本周工作节奏与上周基本持平，保持良好状态！`;
    } else {
    ...
    res.json({ success: true, data: insight } as ApiResponse<WeeklyInsight>);
  } catch (err: any) {
    res.status(500).json({ success: false, data: null, message: err.message });
  }
});
